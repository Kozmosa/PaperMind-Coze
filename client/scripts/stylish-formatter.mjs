...
function getStyleText(color) {
  if (typeof color === 'undefined') {
    return (format, text) => util.styleText(format, text, { validateStream: true });
  }
  if (color) {
    return (format, text) => util.styleText(format, text, { validateStream: false });
  }
  return (_, text) => text;
}
...
export default function (results, data) {
  ...
  results.forEach((result) => {
    ...
    output += `${table(
      ...
      .split('\n')
      .map((el) => el.replace(/(\d+)\s+(\d+)/u, (m, p1, p2) => styleText('dim', `${p1}:${p2}`)))
      .join('\n')}\n\n`;
  });
