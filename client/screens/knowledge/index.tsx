...
export default function KnowledgePage() {
  ...
  const enterSelectMode = (doc: any) => {
    setSelectMode(true);
    setSelectedIds(new Set([`${doc.type || doc.record_type}_${doc.id}`]));
  };
  ...
  const handleGenerateNote = () => {
    setNoteHelperSourceFiles(
      allRecords
        ...
        .map((r) => ({
          id: r.id,
          type: r.record_type as 'study_note' | 'material',
  ...
  const folderTree = useMemo(() => {
    ...
    allRecords.forEach((rec: any) => {
      ...
      parts.forEach((part: string, i: number) => {
        soFar += '/' + part;
        if (!cur.has(part))
          cur.set(part, { name: part, fullPath: soFar + '/', subfolders: new Map(), docs: [] });
  ...
  const countFolder = (node: FolderNode): number => {
    ...
    node.subfolders.forEach((s) => (c += countFolder(s)));
    return c;
  };
    ...
    return l2Nodes
      .map((l2, i) => {
        ...
        return {
          ...
          rx: Math.max(rx, 90),
          ry: Math.max(ry, 70),
          color: L2_CLUSTER_COLORS[i % L2_CLUSTER_COLORS.length],
        };
      })
      .filter(Boolean) as {
      id: string;
      ...
      <View style={{ flex: 1, position: 'relative' }}>
                ...
                {l2Clusters.map((cluster) => (
                  <G key={`zone-${cluster.id}`}>
                    <Circle
                      cx={cluster.cx}
                      cy={cluster.cy}
                      r={Math.max(cluster.rx, cluster.ry)}
                      fill={cluster.color}
                      opacity={0.6}
                    />
                    <SvgText
                      x={cluster.cx}
                      y={cluster.cy - Math.max(cluster.rx, cluster.ry) + 18}
                      textAnchor="middle"
                      fontSize={12}
                      fill={C.text2}
                      fontWeight="600"
        ...
        >
          ...
          <View style={{ flexDirection: 'row', gap: 12 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              <View
                style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: C.primaryLight }}
              />
        ...
        {showLegend && (
          ...
          >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              <View
                style={{ width: 18, height: 1.5, backgroundColor: C.primaryLight, borderRadius: 1 }}
              />
          ...
          ...node.docs.map((doc, i) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => {
                  if (selectMode) toggleSelect(doc);
                  else
                    router.push(doc.type === 'study_note' ? '/study-note-edit' : '/material-edit', {
                      id: doc.id,
                    });
                }}
                onLongPress={() => {
                  if (!selectMode) enterSelectMode(doc);
                }}
              >
                {selectMode && (
                  <Feather
                ...
                <Feather
                  ...
                  size={14}
                  color={doc.type === 'study_note' ? C.primary : C.accent}
                />
                <View style={{ flex: 1 }}>
                  <Text
      ...
      <Screen statusBarStyle="dark" safeAreaEdges={['left', 'right', 'bottom']}>
        <View style={{ flex: 1 }}>
          ...
          <View style={{ paddingTop: insets.top + 12, paddingHorizontal: 20, paddingBottom: 8 }}>
            <Text style={{ fontSize: 30, fontWeight: '800', color: C.text, letterSpacing: -0.5 }}>
              知识库
            </Text>
            {graphData && (
          ...
          {loading ? (
            ...
            >
              ...
              <Text style={{ marginTop: 12, fontSize: 16, color: '#E17055', fontWeight: '600' }}>
                图谱加载失败
              </Text>
              <Text style={{ marginTop: 4, fontSize: 13, color: C.text3, textAlign: 'center' }}>
                {graphError}
              </Text>
              <TouchableOpacity
                style={{
            ...
            >
              ...
              <Text style={{ marginTop: 16, fontSize: 17, color: C.text3, fontWeight: '600' }}>
                暂无知识记录
              </Text>
            ...
            <View style={{ flex: 1 }}>
              <GraphErrorBoundary
                fallback={<FallbackCards graphData={graphData} onSelect={handleSelectTag} />}
              >
                {renderGraph()}
              </GraphErrorBoundary>
            </View>
                ...
                >
                  {folderTree.size === 0 ? (
                    <View style={{ alignItems: 'center', padding: 40 }}>
                      ...
                      <Text style={{ fontSize: 16, color: C.text3, marginTop: 12 }}>
                        暂无文件夹
                      </Text>
                    </View>
                  ) : (
          ...
          >
            ...
            >
              <View style={{ alignItems: 'center', paddingTop: 12, paddingBottom: 8 }}>
                <View
                  style={{ width: 40, height: 4, borderRadius: 2, backgroundColor: C.shadowDark }}
                />
              </View>
              <View
                style={{
                  ...
                  {selectedTag && (
                    <View
                      style={{
                        ...
                        borderRadius: 7,
                        backgroundColor:
                          selectedTag.level === 'L1'
                            ? C.primary
                            : selectedTag.level === 'L2'
                              ? C.primaryLight
                              : C.text3,
                      }}
                    />
              ...
              >
                {loadingTagDocs ? (
                  ...
                  <View style={{ padding: 24, alignItems: 'center' }}>
                    <Text style={{ color: C.text3 }}>暂无关联文档</Text>
                  </View>
                ) : (
                  tagDocuments.map((doc, i) => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        setTagDocuments([]);
                        router.push(
                          doc.type === 'study_note' ? '/study-note-edit' : '/material-edit',
                          { id: doc.id },
                        );
                      }}
                    >
          ...
          >
            ...
            <TouchableOpacity
              style={{
                ...
                alignItems: 'center',
                gap: 6,
                backgroundColor: selectedIds.size > 0 ? C.primary : C.shadowDark,
                paddingHorizontal: 20,
                paddingVertical: 10,
                borderRadius: 20,
              }}
...
function ToggleBtn({
  ...
  return (
    ...
    >
      ...
      <Text style={{ fontSize: 14, fontWeight: '700', color: active ? C.primary : C.text3 }}>
        {label}
      </Text>
    </TouchableOpacity>
  );
    ...
    <View style={{ paddingHorizontal: 16 }}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
        {graphData.nodes.map((node) => (
          ...
          >
            <Text style={{ fontSize: 13, fontWeight: '700', color: C.text }} numberOfLines={2}>
              {node.name}
            </Text>
