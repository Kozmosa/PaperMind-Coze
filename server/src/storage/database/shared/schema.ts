...
export const fileContents = pgTable(
  ...
  (table) => [index('file_contents_draft_id_idx').on(table.draft_id)],
);
...
export const papernoteStyle = pgTable(
  ...
  (table) => [index('papernote_style_user_id_idx').on(table.user_id)],
);
...
export const forums = pgTable(
  ...
  (table) => [index('forums_type_idx').on(table.type)],
);
