...
async function clearExistingData() {
  ...
  console.log(
    `   ✅ 已清理 study_notes (${r1.error ? 'ERR' : 'OK'}), materials (${r2.error ? 'ERR' : 'OK'})\n`,
  );
}
...
async function main() {
  ...
  if (health.status !== 200) {
    console.log(`❌ 后端异常: ${health.status}`);
    process.exit(1);
  }
  ...
  for (let i = 0; i < materialFiles.length; i++) {
    ...
    try {
      ...
      if (ext === '.pdf') {
        ...
        console.log(
          `      PDF文本: ${fileContent.length} 字符, 预览: ${fileContent.slice(0, 80).replace(/\n/g, ' ')}...`,
        );
      } else if (ext === '.pptx') {
        console.log(`   📊 [${i + 1}/${materialFiles.length}] ${fileName} — 解析PPTX...`);
        fileContent = await extractPptxText(filePath);
        console.log(
          `      PPTX文本: ${fileContent.length} 字符, 预览: ${fileContent.slice(0, 80).replace(/\n/g, ' ')}...`,
        );
      } else if (ext === '.ppt') {
        console.log(
          `   📊 [${i + 1}/${materialFiles.length}] ${fileName} — 旧版PPT格式，无法提取...`,
        );
        ...
        console.log(
          `   📄 [${i + 1}/${materialFiles.length}] ${fileName} — 文本预览: ${fileContent.slice(0, 80).replace(/\n/g, ' ')}...`,
        );
      } else {
        // 图片等无法提取的
        fileContent = `[${ext.toUpperCase()} 文件，需OCR: ${fileName}]`;
        console.log(
          `   🖼️  [${i + 1}/${materialFiles.length}] ${fileName} — 无法提取文本 (${ext})`,
        );
      }
      ...
      if (createRes.data?.id) {
        ...
        results.push({
          fileName: `test_data/学习资料/${fileName}`,
          id: createRes.data.id,
          title,
          status: 'processing',
        });
  ...
  const noteResults = results.filter(
    (r) => results.indexOf(r) < mdFiles.length && r.status === 'processing',
  );
  for (const nr of noteResults) {
    ...
    try {
      ...
      console.log(`   ⚠️ ${nr.title.slice(0, 30)} → 触发失败`);
    }
  }
  ...
  for (let round = 0; round < 8; round++) {
    ...
    console.log(
      `   轮次 ${round + 1}/8: ✅ ${done}/${pendingResults.length} 已完成, ⏳ ${pending} 待处理`,
    );
    if (pending === 0) break;
  }
  ...
  try {
    ...
    graphData = gres.data;
    console.log(
      `   ✅ 节点: ${graphData?.nodes?.length || 0} | 边: ${graphData?.edges?.length || 0} | 领域圈: ${graphData?.domains?.length || 0}`,
    );
  } catch {}
  ...
  try {
    ...
    const redDots = [...(nr.data || []), ...(mr.data || [])].filter(
      (r: any) => r.ai_processed && !r.viewed_after_process,
    ).length;
  ...
  const processedNotes = results.filter(
    (r) => results.indexOf(r) < mdFiles.length && r.aiProcessed,
  );
  const processedMats = results.filter(
    (r) => results.indexOf(r) >= mdFiles.length && r.aiProcessed,
  );
